document.addEventListener('DOMContentLoaded', () => {
    ...
    */
    ...
    function renderDetailedExerciseView(exerciseName) {
        ...
        let setsToDisplayInList = []; // Renamed for clarity from 'setsToDisplay'
        if (detailedHistoryViewMode === 'lastDay') {
            toggleHistoryViewBtn.textContent = 'Show All History';
            if (allSetsForExerciseName.length > 0) {
                // allSetsForExerciseName is already sorted with the most recent set at index 0 globally
                ...
                setsToDisplayInList = allSetsForExerciseName.filter(s => {
                    const setDate = s.timestamp instanceof Date ? s.timestamp : new Date(s.timestamp);
                    return getYYYYMMDD(setDate) === targetDateStr;
                });
                // Sort the sets for the target day chronologically (oldest first) for display
                setsToDisplayInList.sort((a, b) => {
            ...
            setsToDisplayInList = [...allSetsForExerciseName].sort((a,b) => {
                const timeA = a.timestamp instanceof Date ? a.timestamp : new Date(a.timestamp);
                const timeB = b.timestamp instanceof Date ? b.timestamp : new Date(b.timestamp);
                return timeA - timeB;
            });
        ...
        // Note: The 1RM calculator population is handled by the existing code *after* this block
        // in the restored script.js. It correctly uses `allSetsForExerciseName`.
        // The chart rendering also uses `allSetsForExerciseName` and is handled later.
